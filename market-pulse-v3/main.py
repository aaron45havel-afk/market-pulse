...
from data_providers import (
    ...
    get_stock_screener_fmp, _test_fmp_connection, STATES, COUNTIES
)
...
@app.get("/api/finance/debug")
async def api_debug():
    """Test FMP API connectivity — shows which endpoints work."""
    if not FMP_API_KEY:
        return JSONResponse({"error": "FMP_API_KEY not set"})
    results = _test_fmp_connection(FMP_API_KEY)
