...
Uses Twelve Data API for live stock prices (free tier, 800 calls/day).
...
- https://api.twelvedata.com/price?symbol=X&apikey=Y (live prices)
...
# ═══════════════════════════════════════════════════
# TWELVE DATA API — Live Stock Prices
# ═══════════════════════════════════════════════════

def _fetch_twelve_data_prices(tickers: list[str], api_key: str) -> dict:
    """
    Fetch live prices from Twelve Data API.
    Free tier: 800 calls/day, 8 calls/minute.
    Batch endpoint supports up to 12 symbols per call.
    Returns {ticker: price} dict.
    """
    if not api_key:
        logger.warning("TWELVE_DATA_API_KEY not set — cannot fetch live prices")
        return {}

    price_map = {}
    batch_size = 12  # Twelve Data free tier max per batch request
    calls_this_minute = 0
    ...
    for i in range(0, len(tickers), batch_size):
        ...
        try:
            ...
            # Multiple symbols returns {"AAPL": {"price": "123.45"}, "MSFT": {"price": "456.78"}}
            if len(batch) == 1:
                ticker = batch[0]
                if isinstance(data, dict) and "price" in data:
                    try:
                        price_map[ticker] = float(data["price"])
                ...
                if isinstance(data, dict):
                    for ticker, val in data.items():
                        if isinstance(val, dict) and "price" in val:
                            try:
                                price_map[ticker] = float(val["price"])
                            except (ValueError, TypeError):
                                pass
            ...
            logger.warning(f"Twelve Data error for batch {i//batch_size}: {e}")

        # Rate limit: max 8 calls per minute on free tier
        if calls_this_minute >= 7:
            logger.info(f"Twelve Data: Rate limit pause. Got {len(price_map)} prices so far.")
            time.sleep(62)  # Wait just over a minute
            calls_this_minute = 0
        else:
        ...
        if (i // batch_size) % 5 == 0 and i > 0:
            logger.info(f"Twelve Data: {len(price_map)} prices from {i + len(batch)} tickers")

    return price_map


def _fetch_twelve_data_quote(ticker: str, api_key: str) -> dict | None:
    """Fetch detailed quote for a single ticker from Twelve Data."""
    if not api_key:
        return None
    try:
...
def build_net_net_screener() -> list[dict]:
    ...
    # Step 4: Pre-filter before fetching prices
    positive_ncav = [c for c in candidates
                     if c["ncav_per_share"] > 0.50
                     and c["shares_outstanding"] > 100000
                     and c.get("current_ratio") and c["current_ratio"] >= 0.5
                     ]
    ...
    # Step 5: Fetch prices via Twelve Data API
    td_api_key = os.getenv("TWELVE_DATA_API_KEY", "")
    tickers = [c["ticker"] for c in positive_ncav]
    price_map = _fetch_twelve_data_prices(tickers, td_api_key)
...
def build_portfolio(capital: float = 50000, num_positions: int = 25) -> dict:
    ...
    # Enrich with quote data from Twelve Data — LIMIT to top 40
    td_api_key = os.getenv("TWELVE_DATA_API_KEY", "")
    enrich_count = min(40, len(initial_candidates))
    td_calls = 0
    for idx, candidate in enumerate(initial_candidates[:enrich_count]):
        quote = _fetch_twelve_data_quote(candidate["ticker"], td_api_key)
        if quote:
            candidate["avg_volume"] = int(quote.get("average_volume", 0) or 0)
            candidate["avg_dollar_volume"] = candidate["avg_volume"] * candidate.get("price", 0)
            candidate["fifty_two_high"] = float(quote["fifty_two_week"]["high"]) if quote.get("fifty_two_week", {}).get("high") else None
            candidate["fifty_two_low"] = float(quote["fifty_two_week"]["low"]) if quote.get("fifty_two_week", {}).get("low") else None
            candidate["exchange"] = quote.get("exchange", "N/A")
        else:
            candidate["avg_volume"] = 0
            candidate["avg_dollar_volume"] = 0
        # Sector not available from Twelve Data quote — keep "N/A"
        candidate.setdefault("sector", "N/A")
        candidate.setdefault("industry", "N/A")
        ...
        if td_calls >= 7:
            ...
            time.sleep(0.5)
    logger.info(f"Portfolio: Enriched {enrich_count} candidates with Twelve Data quotes")
...
def get_net_net_enriched(ticker: str) -> dict | None:
    """Get detailed info for a single stock from Twelve Data."""
    td_api_key = os.getenv("TWELVE_DATA_API_KEY", "")
    quote = _fetch_twelve_data_quote(ticker, td_api_key)
    if not quote:
        return None
    return {
        "sector": "N/A",
        "industry": "N/A",
        "fifty_two_high": float(quote["fifty_two_week"]["high"]) if quote.get("fifty_two_week", {}).get("high") else None,
        "fifty_two_low": float(quote["fifty_two_week"]["low"]) if quote.get("fifty_two_week", {}).get("low") else None,
