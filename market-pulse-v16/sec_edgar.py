...
def build_net_net_screener():
    ...
    for cik, d in financials.items():
        ...
        # Market cap proxy: total assets > $2B means this is a large company, not a net-net candidate
        # Real net-nets have total assets under ~$500M typically
        if ta and ta > 2_000_000_000: skip("too_large"); continue

        # Also filter by NCAV — if total NCAV > $500M, it's a large company
        if ncav > 500_000_000: skip("too_large"); continue

        # Revenue filter — only exclude zero revenue
    ...
    # Take smallest 40 by NCAV + 20 random from the middle range
    candidates.sort(key=lambda x: x["ncav"])
    small = candidates[:40]
    mid = candidates[40:200]
    # Take every 8th from mid range to sample broader
    mid_sample = mid[::8]
    price_candidates = small + mid_sample[:20]
    price_candidates = price_candidates[:60]
...
def build_portfolio(capital=50000, num_positions=25):
    ...
    # Portfolio: stocks with prices, sorted by P/NCAV ascending
    # Start with strict (net-nets), expand if not enough
