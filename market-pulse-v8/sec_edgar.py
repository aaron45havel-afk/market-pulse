...
def build_net_net_screener():
    ...
    for cik, data in financials.items():
        ...
        # Skip negative NCAV and tiny companies
        if ncav_per_share <= 0:
            continue
        if shares < 100000:
            continue
    ...
    # Sort by NCAV per share descending (highest liquidation value first)
    results.sort(key=lambda x: x["ncav_per_share"], reverse=True)

    _write_cache("net_net_screener", results)
    logger.info(f"Net-net screener: {len(results)} companies with positive NCAV")
    return results
